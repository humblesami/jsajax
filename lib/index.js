class AjaxClient {
        ...
        if (processed_result.status != "ok") {
            ...
            if (processed_result.message == "No result") {
                processed_result.message =
                    "Success at " + this.refined_end_point(endpoint)
            }
        }
        return processed_result
    }
    ...
    async post_data_awaitable(
        ...
        return await this.fetch_request(endpoint, "POST", form_data, headers, max_time)
    }
    ...
    post_form(endpoint, formdata, csrf='', headers={}, max_time=120){
        headers['Accept'] = 'application/json';
        headers['X-Requested-With'] = 'XMLHttpRequest';
        if(csrf) {
            headers['X-CSRFToken'] = csrf;
    ...
    post_data(
        ...
        this.fetch_request(endpoint, "POST", form_data, headers, max_time)
    }
}
